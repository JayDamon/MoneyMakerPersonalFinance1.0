 ...
 */
public class NewRecurringTransactionController implements Initializable{
    ...
    @FXML
    ComboBox<Object> cmbAccounts;
    @FXML
    ComboBox<Object> cmbFrequency;
    @FXML
    ComboBox<Object> cmbOccurrence;
    @FXML
    ComboBox<Object> cmbType;

    @FXML
    ComboBox<Object> cmbBudget;
    ...
    @FXML
    Button btnExit, btnAddRecurringTransaction;
    ...
    public void initialize(URL url, ResourceBundle rs) {
        ...
        cmbAccounts.getItems().setAll(sqlAccount.listAccounts());

        cmbBudget.getItems().clear();
        cmbBudget.getItems().setAll(sqlBudget.listActiveBudget());

        cmbFrequency.getItems().clear();
        cmbFrequency.getItems().setAll(sqlMethods.listFrequency());

        cmbOccurrence.getItems().clear();
        cmbOccurrence.getItems().setAll(sqlMethods.listOccurrence());

        cmbType.getItems().clear();
        cmbType.getItems().setAll(sqlMethods.listTranType());
...
//        TODO this doesnt really work for the comboboxes when the "AutoCompleteComboBoxListener" fires because it selects the value after the event handler fires
    ...
    @FXML
    protected void saveNewRecurringTransaction() {
        ...
        if (!areRequiredFieldsEmpty) {
            String account = cmbAccounts.getSelectionModel().getSelectedItem().toString();
            String frequency = cmbFrequency.getSelectionModel().getSelectedItem().toString();
            String occurrence = cmbOccurrence.getSelectionModel().getSelectedItem().toString();
            ...
            if (cmbBudget.getValue() != null) {
                budget = cmbBudget.getSelectionModel().getSelectedItem().toString();
            }
            ...
            sqlTransaction.addRecurringTransaction(transactionName, account, budget, frequency, occurrence, type, startDate, endDate, amount);
